    ...
    def find_empty_pos(self):
        ...
        for line in range(self.board_n_lines()):
            for column in range(self.board_n_columns()):
                ...
                if(Content.is_empty(self.get_content(curr_pos)) == True):
    ...
    def board_moves(self):
        ...
        def find_valid_move(pos: Pos, adj: Adj) -> Move :
            ...
            """ a move is valid if is inner and outter adjecent positions are valid and
                have 'O' as content
            """
            if ((Position.pos_is_valid(adj_inner,self.board)  and Content.is_peg(self.get_content(adj_inner))) and
                (Position.pos_is_valid(adj_outter,self.board) and Content.is_peg(self.get_content(adj_outter)))):
...
def board_perform_move(self, move: Move) -> Board:
    ...
    def find_content_pos(self,content: Content) :
        ...
        positions = [ (l,c) for l in lines for c in columns if self.get_content(Position.make_pos(l,c)) == content]
